...
def apply_individual_formula_on_given_equation(equation, formula_lhs, formula_rhs, do_only_arithmetic=False, structure_satisfy=False):
    ...
    if structure_satisfy:
      return does_given_equation_satisfy_formula_lhs_structure(equation, formula_lhs), copy.deepcopy(variable_list)
...
log += search(eq, 5, [None, "simple2_law.txt", None])

def compute(eq_list, formula):
    final = []
    output = False
    for eq in eq_list:
        tmp = apply_individual_formula_on_given_equation(tree_form(eq), tree_form(formula), None, do_only_arithmetic=False, structure_satisfy=True)
        if tmp[0]:
            output = True
            final.append(tmp[1])
    return final, output
...
lie = """f_noun
 f_past
  f_verb3
   ...
   u_1
 d_lie"""
def reverse(dic, formula):
    for key in dic.keys():
        formula = replace(formula, tree_form(key), dic[key])
    return formula
for event in compute(events[-1], telling)[0]:
    new_event = {"u_0": event["u_0"], "u_1": event["u_2"]}
    k = reverse(new_event, tree_form(knowing))
    if compute(event_list, str_form(k))[1]:
        t = reverse({"u_0": event["u_0"], "u_1": event["u_1"]}, tree_form(truth))
        print(simple_8.process([str_form(t), told]))
    else:
        t = reverse({"u_0": event["u_0"], "u_1": event["u_1"]}, tree_form(lie))
        print(simple_8.process([str_form(t), told]))
